var CanvasForSign = {
    ...
    init: function () {
        ...
        document.getElementById('signature').addEventListener('mousedown', CanvasForSign.sketchpad_mouseDown, false);
        document.getElementById('signature').addEventListener('mousemove', CanvasForSign.sketchpad_mouseMove, false);
        window.addEventListener('mouseup', CanvasForSign.sketchpad_mouseUp, false);

        // React to touch events on the canvas
        document.getElementById('signature').addEventListener('touchstart', CanvasForSign.sketchpad_touchStart, false);
        document.getElementById('signature').addEventListener('touchend', CanvasForSign.sketchpad_touchEnd, false);
        document.getElementById('signature').addEventListener('touchmove', CanvasForSign.sketchpad_touchMove, false);
...
//When onclick "Reserver" button, set canvas hidden to shown
function canvasShow() {
    var etat = document.getElementById("canvasMain").style.display;
    var bouton = document.getElementById("btnReserver");

    //Statut change depends on action
    if (etat === "none") {
        $("#infoShow").fadeOut(750);
        $("#canvasMain").fadeIn(750);
        bouton.value = "Annuler";
        
    } else {
        $("#infoHide").fadeIn(750);
        $("#canvasMain").fadeOut(750);
        $("#btnReserver").hide();
        bouton.value = "Réserver";
    }
}
